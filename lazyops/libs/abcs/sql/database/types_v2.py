...
OT = TypeVar('OT', BasePydantic, BaseORM) # ORM Object Type
PT = TypeVar('PT', bound = BaseORMModel) # Pydantic Model Type
...
class ObjectCRUD(Generic[OT, PT]):

    def __init__(
        self, 
        model: Type[OT], 
        schema: Optional[Type[PT]] = None,
    ...
    async def all(self, db: AsyncSession, stmt: Select) -> List[OT]:
        """
        Returns all objects of the query
        """
        res = (await db.scalars(stmt)).all()
        res = cast(List[self.model], res)
        return res
    ...
    async def remove(self, db: AsyncSession, *, id: Union[int, str]) -> OT:
        """
        Removes an object
        """
    ...
    async def remove_obj(self, db: AsyncSession, *, db_obj: ModelType) -> OT:
        """
        Removes an object
        """
    ...
    async def _cast_results_to_objects(
        ...
        **kwargs,
        ...
        objects: List[OT] = await self.get_many_by_ids(db, ids = object_ids, **kwargs)
        if cast_to_source:

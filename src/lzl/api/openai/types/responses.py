...
class BaseResponse(BaseResource):
    ...
    def metadata_fields(self) -> List[str]:
        ...
        """
        return [
            name for name in get_pydantic_field_names(self) if \
                name not in [
                    "response", "response_data", "input_object",
                    "data_model",  "choice_model", "event_model",
                    "data", "choices", "events"
    ...
    def handle_metadata(
        ...
        **kwargs
        ...
        for field in self.metadata_fields:
            if field in item:
                if field == 'usage':
                    self.usage = Usage.model_validate(item[field])
                    continue
                ...
                setattr(self, field, cast(type(getattr(self, field)), item[field])) # type: ignore
    ...
    def handle_data_item(
        ...
        **kwargs
        ...
        if isinstance(item, dict) and item.get('data'):
            ...
            self.data_model.model_validate(item) if self.data_model else item
    ...
    def handle_choice_item(
        ...
        **kwargs
        ...
        if item.get('choices'):
            ...
            self.choices.append(self.choice_model.model_validate(item))

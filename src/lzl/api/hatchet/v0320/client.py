...
if TYPE_CHECKING:
    ...
    from hatchet_sdk.workflows_pb2 import StickyStrategy
...
class HatchetClient(abc.ABC):
    ...
    def configure_from_env_file(
        ...
        **kwargs,
        ...
        # if app_env and not isinstance(app_env, str): app_env = app_env.name
        # if app_env: app_env = str(app_env).lower()
        logger.info(f'Configuring Hatchet Client from Environment File: {env_file}', prefix = f'Hatchet - {app_env}')
...
Hatchet: HatchetClient = ProxyObject(
    ...
    obj_getter = 'lzl.api.hatchet.utils.get_hatchet_client',
)

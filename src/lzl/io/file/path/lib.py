...
def guess_type(
    path: t.Union[str, "os.PathLike[str]"], strict: bool = True
) -> t.Tuple[t.Optional[str], t.Optional[str]]:
    if sys.version_info < (3, 8):  # pragma: no cover
        path = os.fspath(path)
    return mimetypes_guess_type(path, strict)

def get_file_info(path: t.Union[str, "os.PathLike[str]"]) -> t.Dict[str, t.Any]:
    if sys.version_info < (3, 8):  # pragma: no cover
        path = os.fspath(path)
...
class _Flavour(object):
    ...
    def parse_parts(self, parts: t.List[str]) -> t.Tuple[str, str, t.List[str]]:
        parsed = []
        sep = self.sep
        altsep = self.altsep
        drv = root = ''
            ...
            if drv or root:
                if not drv:
                    ...
                    for part in it:
                        ...
                        if altsep: part = part.replace(altsep, sep)
    ...
    def join_parsed_parts(self, drv: str, root: str, parts: t.List[str], drv2: str, root2: str, parts2: t.List[str]) -> t.Tuple[str, str, t.List[str]]:
        """
...
class _WindowsFlavour(_Flavour):
    ...
    def splitroot(self, part: str, sep: str = sep) -> t.Tuple[str, str]:
        first = part[0:1]
        second = part[1:2]
    ...
    def casefold_parts(self, parts: t.List[str]) -> t.List[str]:
        return [p.lower() for p in parts]

    def compile_pattern(self, pattern: str) -> t.Callable[[str], t.Optional[str]]:
        """
    ...
    def _split_extended_path(self, s: str, ext_prefix: str = ext_namespace_prefix) -> t.Tuple[str, str]:
        """
        Split a path and a possible extended prefix.
        """
        prefix = ''
    ...
    def _ext_to_normal(self, s: str) -> str:
        # Turn back an extended path into a normal DOS-like path
        return self._split_extended_path(s)[1]

    def is_reserved(self, parts: t.List[str]) -> bool:
    ...
    def make_uri(self, path: 'Path') -> str:
    ...
    def gethomedir(self, username: str) -> str:
        """
        Return the home directory of the given username.
        """
...
class _PosixFlavour(_Flavour):
    ...
    def splitroot(self, part: str, sep: str = sep) -> t.Tuple[str, str, str]:
    ...
    def casefold(self, s: str) -> str:
        """
        Return a case-folded version of the given string.
        """
        return s

    def casefold_parts(self, parts: t.List[str]) -> t.List[str]:
        """
        Return a case-folded version of the given list of parts.
        """
        return parts
    ...
    def resolve(self, path: 'Path', strict: bool = False) -> str:
        """
        Resolve a path.
        """
        sep = self.sep
    ...
    def gethomedir(self, username: str) -> str:
        ...
        if not username:
            ...
            import pwd
            try: return pwd.getpwnam(username).pw_dir
            except KeyError:
                raise RuntimeError("Can't determine home directory "
                                   "for %r" % username)
...
class _Selector:
    ...
    def __init__(self, child_parts: t.List[str], flavour: t.Type['PurePath']) -> None:
    ...
    def select_from(self, parent_path: 'Path') -> t.Iterable[t.Tuple[str, t.Any]]:
...
class _TerminatingSelector:
    ...
    def _select_from(self, parent_path: 'Path', is_dir: t.Callable[[str], bool], exists: t.Callable[[str], bool], scandir: t.Callable[[str], t.Iterable[t.Tuple[str, t.Any]]]) -> t.Iterable[t.Tuple[str, t.Any]]:
        yield parent_path
...
_PathT = t.TypeVar('_PathT', bound = 'Path')
...
class Path(PurePath):
    ...
    def __new__(cls: t.Type[_PathT], *args, **kwargs) -> _PathT:
        if cls is Path:
    ...
    def _make_child_relpath(self, part: str) -> _PathT:
    ...
    def iterdir(self) -> t.Iterable[_PathT]:
    ...
    def glob(self, pattern: str) -> t.Iterable[_PathT]:
    ...
    def rglob(self, pattern: str) -> t.Iterable[_PathT]:
    ...
    def resolve(self, strict=False) -> _PathT:
        """
    ...
    def stat(self) -> t.Tuple[int, int, int, int, int, int, int, int, int, int, int, int]:
        """
    ...
    def info(self) -> t.Tuple[int, int, int, int, int, int, int, int, int, int, int, int]:
        """
        Return the result of the stat() system call on this path, like
        os.stat() does.
        """
    ...
    def open(self, mode: str = 'r', buffering: int = -1, encoding: t.Optional[str] = None,
             errors: t.Optional[str] = None, newline: t.Optional[str] = None) -> t.IO[t.Any]:
        """
    ...
    def read_text(self, encoding: t.Optional[str] = None, errors: t.Optional[str] = None) -> str:
        """
    ...
    def write_text(self, data: str, encoding: t.Optional[str] = None, errors: t.Optional[str] = None) -> int:
        """

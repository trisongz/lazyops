...
from .utils import dict_diff, format_table_name
...
from lzl.api.aiorqlite import Connection, Cursor
from lzl.api.aiorqlite.errors import DBError
...
class BaseRqliteDB(abc.ABC):
    """
    The Rqlite Database
    """
    timeout: Optional[float] = 60.0
    _db_kind: Optional[str] = 'RQLite'
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._aio: t.Optional['Connection'] = None
        ...
        self._spawned: t.Dict[str, t.List['Connection']] = {
            'io': [],
            'aio': [],
        }
    ...
    def _get_io_(self, timeout: t.Optional[float] = None) -> 'Connection':
        ...
        new = aiorqlite.connect(
            self.conn_uri, 
            timeout = timeout or self.timeout,
            read_consistency = 'strong',
            default_mode = 'sync',
            default_return = 'cursor',
        )
        self._spawned['io'].append(new)
        return new

    def _get_aio_(self, timeout: Optional[float] = None) -> 'Connection':
        """
    ...
    def _init_aio_(self) -> None:
        ...
        self._validate_table_(self._aio)
        if not self.is_configured:
            self._is_configuring = True
            self.run_configure(self._aio)
            self._is_configuring = False
    ...
    @property
    def aio(self) -> 'Connection':
        """
        [Async] Returns the IO Connection
        """
        if self._aio is None: self._init_aio_()
        return self._aio
    ...
    @property
    def _asql_debug(self) -> t.Callable[..., t.Awaitable['Cursor']]:
        """
    ...
    def _sqlmany_debug(self) -> t.Callable[..., 'Cursor']:
        ...
        def _executemany(statement, *args, **kwargs):
            logger.info(f'SQL: {statement}: {args} {kwargs}')
            return self._io.executemany2(statement, *args, **kwargs)
        return _executemany
    ...
    @property
    def _asqlmany(self) -> t.Callable[..., t.Awaitable['Cursor']]:
        """
        [Async] Returns the SQL
        """
    ...
    def _check_table_configured_(self, conn: t.Optional['Connection'] = None) -> bool:
        ...
        try:
            result = conn.execute(f'SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type = "table" AND name = "{self.table}")', is_async = False).results

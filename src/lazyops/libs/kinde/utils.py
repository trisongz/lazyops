...
_kinde_kdbs: Optional[Dict[str, 'KVDBSession']] = {}
...
def get_kinde_kdb_session(
    ...
    **kwargs,
    ...
    """
    Returns the Kinde KVDB Session
    """
    global _kinde_kdbs
    if name is None: name = 'kinde.global'
    if url is None: url = os.getenv('KINDE_REDIS_URL')
    if name not in _kinde_kdbs:
        from kvdb import KVDBClient
        _kinde_kdbs[name] = KVDBClient.get_session(name = name, url = url, serializer = serializer, **kwargs)
    return _kinde_kdbs[name]
...
def normalize_url_name(name: str) -> str:
    """
    Normalizes the audience name to transform the url

    >>> normalize_url_name('https://domain.us.auth0.com/userinfo')
    'domain-us-auth0-com-userinfo'
    """
    return name.replace('https://', '').replace('http://', '').replace('/', '-').replace('.', '-').replace(' ', '-').lower()
